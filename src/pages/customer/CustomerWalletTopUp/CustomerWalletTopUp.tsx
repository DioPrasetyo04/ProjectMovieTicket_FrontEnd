...
const CustomerWalletTopUp = () => {
  ...
  const setUpBalance = (value: number) => {
    setBalance(value);
  };
          ...
          <div className="grid grid-cols-3 gap-4">
            {OPTIONS_TOPUP.map((item, i) => (
              <button
                ...
                key={`${item + i}`}
                className="group relative theather-card flex flex-col rounded-3xl p-4 gap-[2px] bg-white/10 backdrop-blur-md hover:bg-purple-600 has-checked:bg-purple-700 transition-all duration-300"
              >
                <input
                  type="radio"
                  name="amount"
                  ...
                  value={item}
                  //   props tambahan dari react js untuk properties css checked
                  checked={balance === item}
                  required
                />
...
export default CustomerWalletTopUp;
