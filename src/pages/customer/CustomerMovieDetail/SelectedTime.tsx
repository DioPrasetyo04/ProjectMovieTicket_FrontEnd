      ...
      <form action="choose-seat.html" className="relative px-5 mt-5">
        <div id="Theaters" className="tab-content flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            {detail?.times.map((time, i) => (
              <button
                type="button"
                key={`${time + i}`}
                onClick={() => {
                  const selected = `${dayjs().add(1, "day").format("YYYY-MM-DD")} ${time}`;
                  console.log(selected);
                  setDate(selected);
                }}
                className="group relative theather-card flex flex-col rounded-3xl p-4 gap-[2px] bg-white/10 backdrop-blur-md has-disabled:!bg-white/10 hover:bg-purple-500 has-checked:bg-purple-600 transition-all duration-300"
              >
                <input
                  type="radio"
                  name="Time"
                  value={`${dayjs().add(1, "day").format("YYYY-MM-DD")} ${time}`}
                  checked={
                    date ===
                    `${dayjs().add(1, "day").format("YYYY-MM-DD")} ${time}`
                  }
