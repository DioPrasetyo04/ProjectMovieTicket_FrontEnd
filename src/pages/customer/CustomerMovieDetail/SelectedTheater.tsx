    ...
    >
      ...
      <div className="relative px-5 mt-5">
        <div id="Theaters" className="tab-content flex flex-col gap-4">
          ...
          {detail.theaters.map((t) => {
            const selected = theater?._id === t._id;

            return (
              <label
                key={t._id}
                className={`relative flex items-center rounded-3xl p-4 gap-2 backdrop-blur-md
              ...
              >
                <input
                  ...
                  value={t._id}
                  checked={selected}
                  onChange={() => setTheater(t)}
                  className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
                  required
                />

                <div className="w-[120px] h-[130px] flex shrink-0 rounded-2xl overflow-hidden bg-[#D9D9D9]">
                  <img
                ...
                <div className="flex flex-col">
                  <h3 className="font-semibold line-clamp-2">{t.name}</h3>
                  <p className="text-sm text-premiere-grey">{t.city}</p>
                </div>
              </label>
            );
          })}
