...
const DetailMovie = () => {
  ...
  const setTabView = (tab: "about" | "reviews" | "theaters" | "cast") => {
    setTab(tab);
  };

  const { detail } = useLoaderData() as LoaderData;

  //   dispatch aktivate set step action dari react redux
          ...
          >
            <SwiperSlide className="swiper-slide !w-fit py-[1px]">
              <button
                type="button"
                className="tab-link"
                onClick={() => setTabView("about")}
              >
                <div
                  className={`flex rounded-full p-[12px_14px] bg-[#FFFFFF1A] font-semibold text-sm hover:ring-1 hover:ring-white transition-all duration-300 ${tab === "about" ? "!bg-white text-premiere-black" : "text-white"}`}
                >
                  About
                </div>
              </button>
            ...
            <SwiperSlide className="swiper-slide !w-fit py-[1px]">
              ...
              >
                <div
                  className={`flex rounded-full p-[12px_14px] bg-[#FFFFFF1A] font-semibold text-sm hover:ring-1 hover:ring-white transition-all duration-300 ${tab === "reviews" ? "!bg-white text-premiere-black" : "text-white"}`}
                >
                  Reviews
                </div>
              </button>
            ...
            <SwiperSlide className="swiper-slide !w-fit py-[1px]">
              <button
                type="button"
                className="tab-link"
                onClick={() => setTabView("theaters")}
              >
                <div
                  className={`flex rounded-full p-[12px_14px] bg-[#FFFFFF1A] font-semibold text-sm hover:ring-1 hover:ring-white transition-all duration-300 ${tab === "theaters" ? "!bg-white text-premiere-black" : "text-white"}`}
                >
                  Theaters
                </div>
              </button>
            ...
            <SwiperSlide className="swiper-slide !w-fit py-[1px]">
              <button
                type="button"
                className="tab-link"
                onClick={() => setTabView("cast")}
              >
                <div
                  className={`flex rounded-full p-[12px_14px] bg-[#FFFFFF1A] font-semibold text-sm hover:ring-1 hover:ring-white transition-all duration-300 ${tab === "cast" ? "!bg-white text-premiere-black" : "text-white"}`}
                >
                  Casts
                </div>
              </button>
      ...
      <div id="Bottom-Nav" className="relative w-full h-[123px] flex shrink-0">
        <div className="fixed bottom-5 left-5 right-5 w-full max-w-[330px] mx-auto flex items-center justify-between rounded-full p-[10px_14px] pl-6 gap-[14px] bg-[#FFFFFF33] z-20 backdrop-blur-md">
          <div className="flex justify-center gap-3 items-center">
            ...
            <span className="text-sm mt-[2px] font-bold">/ orang</span>
          </div>
          <button
            type="button"
