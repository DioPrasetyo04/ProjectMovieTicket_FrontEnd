...
import { deleteCustomer } from "@/services/customers/customer.service";
import { deleteGenre } from "@/services/genre/genre.service";
import type { ActionColumnCustomerProps } from "@/Types/ActionColumn";
...
const ActionColumn = ({ email }: ActionColumnCustomerProps) => {
  const { setAllert } = useAllert();
  const [confirmationDelete, setConfirmationDelete] = useState(false);
  const revalidator = useRevalidator();

  const { isPending, mutateAsync } = useMutation({
    mutationFn: () => deleteCustomer(email),
    onSuccess: (data) => {
      const status = (data.status || "Success") as
        ...
        | "Failed"
        | "Info";
      const message = data.message || "Customer deleted successfully";
  ...
  const handleDelete = async () => {
    try {
      ...
      toast.error("Customer data failed deleted, something went wrong");
    }
  };
  ...
  return (
    <div className="inline-flex items-center gap-4 p-5">
      ...
      <Button size="sm" variant="secondary" asChild>
        <Link to={`/admin/customer/edit/${email}`}>
